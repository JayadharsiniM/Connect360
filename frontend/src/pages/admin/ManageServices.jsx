    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-stack-md">
        <div className="flex flex-col gap-stack-xs">
          ...
          <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="input-field" placeholder="e.g., Plumbing" required />
        </div>
        <div className="flex flex-col gap-stack-xs">
          <label className="font-hanken text-label-md text-on-surface">Icon (Material Symbol name)</label>
          <div className="flex items-center gap-2">
            <input type="text" value={formData.icon} onChange={(e) => setFormData({ ...formData, icon: e.target.value })} className="input-field flex-1" placeholder="e.g., plumbing, electrical_services" />
            {formData.icon && (
      ...
      <div className="flex flex-col gap-stack-xs">
        ...
        <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="input-field" rows={2} placeholder="Brief description of the service" />
      </div>
    ...
    <DashboardLayout>
      ...
      <div className="lg:hidden pt-6 md:pt-24 px-margin-mobile md:px-margin-desktop max-w-container mx-auto flex flex-col gap-stack-lg pb-24">
        ...
        {services.length > 0 ? (
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-lg bg-primary-container flex items-center justify-center flex-shrink-0">
                            <span className="material-symbols-outlined text-primary text-[18px]">{service.icon || 'category'}</span>
                          </div>
                          <span className="font-hanken text-body-md text-on-surface font-medium">{service.name}</span>
                        </div>
                      </td>
                      <td className="px-5 py-4 font-hanken text-body-sm text-on-surface-variant hidden md:table-cell max-w-xs truncate">{service.description || '—'}</td>
                      <td className="px-5 py-4 font-hanken text-body-sm text-on-surface-variant">{service.icon || '—'}</td>
                      <td className="px-5 py-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => startEdit(service)} className="p-2 text-on-surface-variant hover:text-primary rounded-lg hover:bg-primary-container/30 transition-colors" title="Edit" aria-label={`Edit ${service.name}`}>
                            <span className="material-symbols-outlined text-[18px]">edit</span>
                          </button>
                          <button onClick={() => handleDelete(service.id, service.name)} className="p-2 text-on-surface-variant hover:text-error rounded-lg hover:bg-error-container/30 transition-colors" title="Delete" aria-label={`Delete ${service.name}`}>
          ...
          <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-12 text-center shadow-level-1">
            ...
            <p className="font-hanken text-body-md text-on-surface-variant">Add service categories to get your platform started.</p>
            <button onClick={() => setShowForm(true)} className="btn-primary mt-4">Add First Service</button>
          </div>
        )}
      </div>
      ...
      <div className="hidden lg:flex lg:flex-col gap-stack-lg">
        ...
        <div className="flex justify-between items-end">
          <div>
            <h2 className="font-manrope text-headline-lg text-primary">Manage Services</h2>
            <p className="font-hanken text-body-md text-on-surface-variant mt-1">View, add, and configure platform service categories.</p>
          </div>
          {!showForm && (
            <button onClick={() => setShowForm(true)} className="bg-primary-container text-on-primary font-hanken text-label-md px-6 h-12 rounded-lg hover:bg-primary transition-colors flex items-center gap-2 shadow-sm">
        ...
        {/* Data table */}
        {services.length > 0 ? (
          <div className="bg-surface-container-lowest rounded-xl border border-outline-slate overflow-hidden flex flex-col">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-surface-container border-b border-outline-slate">
                    <th className="py-4 px-6 font-hanken text-label-md text-primary">Service Name</th>
                    <th className="py-4 px-6 font-hanken text-label-md text-primary">Description</th>
                    <th className="py-4 px-6 font-hanken text-label-md text-primary">Icon</th>
                    <th className="py-4 px-6 font-hanken text-label-md text-primary text-right">Actions</th>
                  </tr>
                </thead>
                <tbody className="font-hanken text-body-sm text-on-surface divide-y divide-surface-container">
                  {services.map((service) => (
                    <tr key={service.id} className="hover:bg-surface-bright/50 transition-colors group">
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg bg-secondary-container/10 flex items-center justify-center text-secondary">
                            <span className="material-symbols-outlined">{service.icon || 'category'}</span>
                          </div>
                          <span className="font-medium text-primary">{service.name}</span>
                        </div>
                      </td>
                      <td className="py-4 px-6 text-on-surface-variant max-w-md truncate">{service.description || '—'}</td>
                      <td className="py-4 px-6 text-on-surface-variant font-mono">{service.icon || '—'}</td>
                      <td className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => startEdit(service)} className="p-1.5 text-on-surface-variant hover:text-secondary rounded hover:bg-secondary-container/10 transition-colors" title="Edit">
                            <span className="material-symbols-outlined text-[20px]">edit</span>
                          </button>
                          <button onClick={() => handleDelete(service.id, service.name)} className="p-1.5 text-on-surface-variant hover:text-error rounded hover:bg-error-container/50 transition-colors" title="Delete">
            ...
            <div className="border-t border-outline-slate p-4 flex items-center justify-between bg-surface-bright">
